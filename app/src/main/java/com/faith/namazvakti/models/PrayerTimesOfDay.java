package com.faith.namazvakti.models;
...
import com.faith.namazvakti.database.Database;
...
import static com.faith.namazvakti.models.PrayerTimeType.Fajr;
import static com.faith.namazvakti.models.PrayerTimeType.Isha;
...
public class PrayerTimesOfDay {
    ...
    public PrayerTimesOfDay(@NonNull LocalDate date,
        ...
        this(
            date,
            prayerTimesOfDay.get(Fajr),
            prayerTimesOfDay.get(Shuruq),
    ...
    public PrayerTimeType currentPrayerTimeType()
    {
        PrayerTimeType nextType = nextPrayerTimeType();
        switch (nextType) {
            case Fajr:    return Isha;
            case Shuruq:  return Fajr;
            case Dhuhr:   return Shuruq;
            case Asr:     return Dhuhr;
            case Maghrib: return Asr;
            case Isha:    return Maghrib;
            case Qibla:   return Isha;
            default:      return Fajr;
        }
    }
    ...
    public static ArrayList<PrayerTimesOfDay> getPrayerTimesForMonthly(Context context, Place place) {
        ArrayList<PrayerTimesOfDay> prayerTimesForMonthly = new ArrayList<PrayerTimesOfDay>();

        for(int i = 0; i < 31; i++) {
            Optional<PrayerTimesOfDay> prayerTimes = getPrayerTimesForDay(context, place, LocalDate.now().plusDays(i));
            if( !prayerTimes.isEmpty() )
                prayerTimesForMonthly.add(prayerTimes.get());
        }

        return prayerTimesForMonthly;
    }
